...
leggi_e_formatta_dati <- function(input_path) {
  ...
  colnames(dati) <- c("Modality", "OMP", "MPI", "tree_creation_time", "communication_time", "rb_search_time", "program_execution_time", "speedup", "efficiency")
  dati$speedup[dati$Modality == "Sequential"] <- "-"
  dati$efficiency[dati$Modality == "Sequential"] <- "-"
